  ...
  xsession = {
    ...
    windowManager.i3 ={
      ...
      config = {
        ...
        startup = map (x: { command = x; always = true; notification = false; }) [
        ...
        keybindings = with builtins;
          let
            attr = buttons: cmd: { name = "${modifier}+${buttons}"; value = cmd; };

            numbersGenerator = sc: cmd: x: let str = toString x; in attr (sc + str) (cmd + str);
            withNumbers = sc: cmd: listToAttrs (tail (genList (numbersGenerator sc cmd) 7));

            dircGenerator = butt: cmd: d:
              attr (butt + lib.toUpper (substring 0 1 d) + substring 1 (stringLength d - 1) d) "${cmd} ${d}";
            withDirections = butt: cmd: listToAttrs (map (dircGenerator butt cmd) [ "left" "down" "up" "right" ]);

            screenshot = flags: "exec maim -u ${flags} | xclip -selection clipboard -t image/png";
          in
